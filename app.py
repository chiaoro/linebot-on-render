...
def handle_message(event):
    ...
    if text == "院務會議請假":
        return line_bot_api.reply_message(event.reply_token,
            FlexSendMessage("院務會議請假", meeting_flex_bubble))
    if text == "夜點費申請":
    ...
    if text in clinic_buttons_text:
        ...
        return line_bot_api.reply_message(event.reply_token,
            TextSendMessage(text="📅 請輸入原門診日期（如5/6上午診）"))
    if text in duty_buttons_text:
        user_sessions[user_id] = {"step":1,"type":text}
        return line_bot_api.reply_message(event.reply_token,
            TextSendMessage(text="🟡 請問值班班別？"))
    if text == "在職證明申請":
        return line_bot_api.reply_message(event.reply_token,
            TextSendMessage(text="📄 在職證明申請表：https://docs.google.com/forms/d/e/1FAIpQLSeI64Av1Fb2Qgm6lCwTaUyvFRejHItS5KTQNujs1rU3NufMEA/viewform"))
    if text == "專師每日服務量填寫":
        return line_bot_api.reply_message(event.reply_token,
            TextSendMessage(text="📋 專師每日服務量填寫表單：https://forms.office.com/Pages/ResponsePage.aspx?id=qul4xIkgo06YEwYZ5A7JD8YDS5UtAC5Gqgno_TUvnw1UQk1XR0MyTzVRNFZIOTcxVVFRSFdIMkQ1Ti4u"))
    if text == "Temp傳檔":
        return line_bot_api.reply_message(event.reply_token,
            TextSendMessage(text="📂 Temp傳檔表單：https://docs.google.com/forms/d/e/1FAIpQLSexoPBHmJYpBlz_IIsSIO2GIB74dOR2FKPu7FIKjAmKIAqOcw/viewform"))
    if text == "外科醫師休假登記表":
        return line_bot_api.reply_message(event.reply_token,
            TextSendMessage(text="📅 外科醫師休假登記表單：https://docs.google.com/forms/d/e/1FAIpQLScT2xDChXI7jBVPAf0rzKmtTXXtbZ6JFFD7EGfhmAvwSVfYzQ/viewform"))
    return line_bot_api.reply_message(event.reply_token,
        TextSendMessage(text="⚠️ 無效指令，請輸入「主選單」重新開始。"))
